...
import { Kosugi_Maru, Quicksand } from 'next/font/google';

const kosugiMaru = Kosugi_Maru({ subsets: ['latin', 'cyrillic', 'latin-ext'], weight: '400' });

export default ({ name, rare }: { name: string, rare: string }) =>
	<div className={`${rare} ${kosugiMaru.className} line-height-normal w-full border-1 border-solid rounded-full text-center box-border`}>
		{name}
		<style jsx scoped>{`
        .Normal {
